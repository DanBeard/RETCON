...
class MeshchatHandle():
    ...
    def start_meshchat(cls, iface, ssid, retcon_config):
        ...
        cls._singleton = subprocess.Popen(
            Template(restart_template).render(command=f"find {dir_path}/../storage -type f -name '*.ratchets' -delete && python {dir_path}/../apps/reticulum-meshchat/meshchat.py --headless --host {ip} --storage-dir {dir_path}/../storage --reticulum-config-dir $HOME/.meshchat-rns"), 
